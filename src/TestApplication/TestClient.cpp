...
namespace ItSoftware::CppIncludeLinux::TestClient
{
    //
    // using
    //
    ...
    //
    // (i): Prints out settings information to stdout.
    //
    void PrintSettings(const AppSettings& settings)
    {
        if (!settings.NoColorOutput) { cout << COLOR_RESET << COLOR_GREEN; }
...
//
// (i): Application entry point.
//      Redirects to ItSoftware::CppIncludeLinux::TestClient::main.
//
int main(int argc, const char* argv[])
{
    return ItSoftware::CppIncludeLinux::TestClient::main(argc, argv);
}